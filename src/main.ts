...
Deno.serve({ hostname: HOST, port: PORT }, async (req, info) => {
  ...
  const username =
    (await (await fetch(`https://sessionserver.mojang.com/session/minecraft/profile/https://sessionserver.mojang.com/session/minecraft/profile/${uuid}`)).json()).name || "!!Unknown Username!!";
